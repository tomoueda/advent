...
loop = set()
counted = set()
def print_graph(just_maze=False):
    newstr = ''
    for y in range(len(lines)):
        for x in range(len(lines[0])):
            if just_maze:
...
fills = {(1, 0): '-', (-1, 0): '-', (0, 1): '|', (0, -1): '|'}
loop.add(fill[0])
loop.add(start)
while fill[0] != start:
    node, d = fill[0], fill[1]
    loop.add(node)
    maze[node] = fills[d]
    x, y = node[0] + d[0], node[1] + d[1]
    new_node = (x, y)
    loop.add(new_node)
    new_dir = next_nodes(node, new_node) 
    fill = ((new_node[0] + new_dir[0], new_node[1] + new_dir[1]), new_dir)
...
def traverse(node):
    ...
    while len(nodes) != 0:
        node = nodes.pop()
        if node not in local_visited and node not in loop:
            visited.add(node)
            ...
            for d in dirs:
                ...
                if (x, y) not in maze:
                    return 0
                nodes.append((x, y))
    counted.update(local)
    return s

n = 0
for y in range(len(lines)):
    for x in range(len(lines[0])):
        if (x, y) not in loop and maze[(x, y)] != '.' and (x, y) not in visited:
            n += traverse((x, y))

print_graph()
print('\n')
print_graph(just_maze=True)
