...
dir = [(1, 1), (-1, -1), (1, 0), (-1, 0), (0, 1), (0, -1), (1, -1), (-1, 1)]
s = 0
for d, curr in nums:
    def should_count():
        for c in curr:
...
reverse = defaultdict(lambda: None)
for d, curr in nums:
    for c in curr:
        reverse[c] = (d, curr)
for y in range(l):
    for x in range(l):
        ...
        if c == '*':
            for d in dir:
                x1 = x + d[0] 
                y1 = y + d[1] 
                if (x1, y1) not in seen and (x1, y1) in reverse:
                    d, curr = reverse[(x1, y1)]
                    items.append(d)
                    seen.update(curr)
        if len(items) == 2:
            s += int(items[0]) * int(items[1])
print(s)
