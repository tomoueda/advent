...
lines = ac.split('\n\n')
instructions_inpt, xmas_str = lines[0], lines[1]

from collections import defaultdict
from tools import V
instructions = defaultdict(list)
for instruction_str in instructions_inpt.split('\n'):
    key_and_ins = instruction_str.split('{')
    key = key_and_ins[0]
    ins = key_and_ins[1][:len(key_and_ins[1]) -1]
    instrs_str = ins.split(',')
    for s in instrs_str[:len(instrs_str) - 1]:
        elems = s.split(':')
        dest = elems[1]
        ...
        if '<' in elems[0]:
            ...
            letter = subelems[0]
            instructions[key].append((letter, '<', int(subelems[1]), dest))
    ...
    instructions[key].append((other, None, None, None))

import re
p = re.compile(r'\d+')
getnum = {'x': 0, 'm': 1, 'a': 2, 's': 3}
s = V(0, 0, 0, 0)
for xmas in xmas_str.split('\n'):
    xmas = tuple(map(int, p.findall(xmas)))
    node = 'in'
    while node != 'A' and node != 'R':
        node = instructions[node]
        for ins, op, comp, dest in node:
            if op == None:
                node = ins
                break
...
less_than = [4001, 4001, 4001, 4001]
greater_than = [0, 0, 0, 0]
nodes = [('in', less_than, greater_than, 'in')]
visited = set()
accepteds = []
# starting part 2
while len(nodes) != 0:
   node, less_than, greater_than, path = nodes.pop()
   if node == 'A':
       print(less_than, greater_than)
       print(path)

       accepteds.append((less_than, greater_than))
       continue
   if node == 'R':
#       print('rejected')
#       print(less_than, greater_than)
#       print(path)
   ...
   if node not in visited:
       ...
       for letter, op, comp, dest in ins:

            if op is None:
                nodes.append((letter, less_than, greater_than, path + '->' + letter))
                continue


            wo_less_than = list(less_than)
            wo_greater_than = list(greater_than)
            w_less_than = list(less_than)
            w_greater_than = list(greater_than)

            # split with and without
            if op == '<':
                w_less_than[getnum[letter]] = min(w_less_than[getnum[letter]], comp)
                wo_greater_than[getnum[letter]] = max(wo_greater_than[getnum[letter]], comp - 1)
            if op == '>': 
                w_greater_than[getnum[letter]] = max(w_greater_than[getnum[letter]], comp)
                wo_less_than[getnum[letter]] = min(wo_less_than[getnum[letter]], comp + 1)
            nodes.append((dest, w_less_than, w_greater_than, path + '->' + dest))

            less_than = list(wo_less_than)
            greater_than = list(wo_greater_than)
s = 0
less_than = [4001, 4001, 4001, 4001]
greater_than = [1, 1, 1, 1]
for accepted in accepteds:
    m = 1 
    lt, gt = accepted
    print(lt, gt)
    for i in range(len(less_than)):
        less_than[i] = min(less_than[i], lt[i])
        greater_than[i] = max(greater_than[i], gt[i])
        m *= (lt[i] - 1) - gt[i]
    s += m
print(s)
